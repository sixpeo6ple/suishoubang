<template>
	<view>
		<view class="comb">
			<text class="title">取货地点</text>
			<text class="value">{{ info }}</text>
			<text class="title">请求者</text>
		...
		<view class="pic" v-if="sign != '3'">
			<text class="title">外卖位置</text>
			<view class="img_box">
				<image class="img" :src="picurl" mode="aspectFill" @click="enlarge(picurl)"></image>
			</view>
		</view>
		<view class="bottom">
			<button class="sub" @click="fcancel" v-if="cancel == 0">取消订单</button>
			<button class = "sub" @click="ffinish" v-if="sign != '3' && cancel == 0">结束订单</button>
			<text class="over" v-else>订单结束</text>
		</view>
	</view>
</template>
